          ...
          {profiles.map((profile, index) => {
            ...
            return (
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className={`p-6 rounded-lg bg-slate-800/30 border border-slate-700 hover:border-current transition-all backdrop-blur-sm group`}
                title={profile.name}
              >
                <IconComponent className={`w-8 h-8 mb-3 mx-auto transition-all ${profile.color}`} />
                <p className="font-semibold text-white mb-1 group-hover:text-inherit">{profile.name}</p>
